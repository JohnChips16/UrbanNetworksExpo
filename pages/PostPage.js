...
const fetchComment = async () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const token = await AsyncStorage.getItem('token');
        const res = await fetch(`http://localhost:9000/v1/users/school/post/${postId}`, {
          method: "GET",
        });
        const responseData = await res.json();
  ...
  const renderJobItemLinked = ({ item }) => {
    const renderHashtags = () => (
      item.hashtags && item.hashtags.length > 0 ? <Text style={styles.hashtags}>{item.hashtags.join(', ')}</Text> : null
    );

    const renderCaption = () => (
      item.attachment ? <Text style={[styles.description, { marginBottom: 10 }]}>{item.caption ? item.caption : 'No data'}</Text> :
      <Text style={[styles.description, { marginBottom: 10, fontSize: item.caption && item.caption.length > 50 ? 14 : 18 }]}>{item.caption ? item.caption : 'No data'}</Text>
    );
    ...
    <ScrollView>
      ...
      {comment.length === 0 ? null : (
        <FlatList
          ...
          keyExtractor={(item, index) => index.toString()}
          renderItem={({ item }) => {
            const correspondingReply = Array.isArray(reply) ? reply.find(replyItem => replyItem.commentId === item._id) : undefined;
            return <RenderCommentItem item={item} item2={correspondingReply} openModal={openModal} />;
          }}
        />
      )}
      <ModalComponent parentId={selectedCommentId} modalVisible={modalVisible} setModalVisible={setModalVisible} />
    </ScrollView>
