  ...
  if (secondsDifference < minute) {
   ...
   const fetchAllPost = async () => {
    try {
      ...
      const res = await fetch("http://localhost:9000/v1/users/fetchmy/post", {
        method: "GET",
...
const renderJobItemLinked = ({ item, navigation }) => {
  ...
  {item.author && (item.author.fullname || item.author.schoolOrUniversityName)}
      ...
      {renderHashtags()}
      <View style={{ padding: 0.5, backgroundColor: '#eee' }}></View>
         <TouchableOpacity onPress={() => navigation.navigate("PostPage1", { postId: item._id })} style={styles.applyButton}>
