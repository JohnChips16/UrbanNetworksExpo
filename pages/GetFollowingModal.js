...
const GetFollowing = ({ parentId, modalVisible, setModalVisible }) => {
...
const navigation = useNavigation();
  ...
  const fetchReplies = async () => {
    try {
      ...
      const res = await fetch(`http://localhost:9000/v1/users/foreign/get/following/${parentId}`, {
        method: "GET",
        headers: { Authorization: `Bearer ${token}` }
      });
      const responseData = await res.json();
      setReplies(responseData.following || []);
    } catch (err) {
  ...
  return (
    ...
    <KeyboardAvoidingView style={styles.modalContent} behavior="padding" enabled>
      <ScrollView style={styles.scrollView}>
        <Text style={{ textAlign: 'center', fontSize: 16, marginTop: 15, }}>Following</Text>
